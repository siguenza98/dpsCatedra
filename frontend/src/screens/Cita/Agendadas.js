...
import CitasDetalles from './Detalles';
const Stack = createNativeStackNavigator();

const Agendadas = ({ route, navigation }) => {
    const [citas, setCitas] = useState([]);

    var usuario = route.params;

    const getCitasAgendadas = () => {
        setCitas('');
        //Conexion a la api
        fetch('http://127.0.0.1:8000/api/citasagendadas', {
            method: 'POST',
            headers: {
    ...
    const detallesCita = (cita) => {
        navigation.push('VeterinarioNavigation', {
            screen: 'Citas Detalles',
            params: { usuario, cita },
        })
    }
